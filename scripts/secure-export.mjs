...
const staticPages = new Set(['index.html', 'hogar.html', 'oficina.html', 'quienes-somos.html']);
const basePath = (process.env.NEXT_PUBLIC_BASE_PATH || '').replace(/\/$/, '');
const enhancement = readFileSync('public/site-interactions.js', 'utf8');
const enhancementName = 'site-interactions-' + createHash('sha256').update(enhancement).digest('hex').slice(0, 12) + '.js';
...
for (const name of readdirSync(directory).filter((name) => name.endsWith('.html'))) {
  ...
  if (staticPages.has(name)) {
    if (/<(?:form|button|input|select|textarea)\b/i.test(html)) {
      throw new Error(name + ': interactive controls require hydration; remove this page from staticPages.');
    }
    ...
    html = html.replace(/<link\b[^>]*>/gi, (tag) =>
      /rel=["']modulepreload["']/i.test(tag) || /as=["']script["']/i.test(tag) ? '' : tag);
    html = html.replace('</body>', '<script defer src="' + basePath + '/' + enhancementName + '"></script></body>');
