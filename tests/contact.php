...
$challenge = ['token' => str_repeat('a', 64), 'issued' => time() - 3];
...
$run = static function ($input, $request = null) use ($directory, $server, $deliver, $challenge) { return \Aquapuel\Contact\handle_request($request ?? $server, is_string($input) ? $input : json_encode($input), $directory, $deliver, $challenge); };
try {
    ...
    expect($run(array_merge($data, ['_token' => 'invalid']))[0] === 403 && count($calls) === 0, 'Reject token forgery without mail');
    expect(!\Aquapuel\Contact\valid_challenge($challenge, $data['_token'], $challenge['issued'] + 1), 'Reject immediate automation');
    expect(!\Aquapuel\Contact\valid_challenge($challenge, $data['_token'], $challenge['issued'] + 1801), 'Reject expired token');
    expect(!\Aquapuel\Contact\valid_challenge([], $data['_token'], time()), 'Reject absent session');
    ...
    $failed = \Aquapuel\Contact\handle_request($server, json_encode($data), $directory, static function () { return false; }, $challenge);
